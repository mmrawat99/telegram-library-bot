...
# ================= CONFIG =================
TOKEN = "8521894848:AAH1x07Cfojnw1korW0FZi6MBxdpqzhqRNI"
...
async def handler(update, context):
    ...
    if lvl=="BOOKS" and t=="OTHER BOOKS 📚":
        ...
        kb.append(["🔙 BACK"])
        return await update.message.reply_text("📚 Other Books 👇",
            reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
    ...
    if lvl=="HELP" and t=="📖 Guide":
        return await update.message.reply_text(
            "🙏 Ye bot M M RAWAT ne bnaya h jisme aap Sadgurudev ke sbhi books magazine or video aaram se receive kr skte h 🙏"
        )
...
async def on_startup(app):
    load_data()
    await verify_storage(app.bot)

if __name__=="__main__":
    app=ApplicationBuilder().token(TOKEN).post_init(on_startup).build()
    app.add_handler(CommandHandler("start", start))
    app.add_handler(MessageHandler(filters.ChatType.CHANNEL, channel_handler))
    app.add_handler(MessageHandler(filters.Regex("🔙 BACK"), back_button_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("🔙 BACK"), handler))
